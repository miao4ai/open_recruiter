...
configure_llm() {
    ...
    case "$choice" in
        ...
        3)
            LLM_PROVIDER="gemini"
            echo ""
            echo -e "  ${MUTED}Get your API key at: https://aistudio.google.com/apikey${NC}"
            ...
            if [[ -z "$LLM_API_KEY" ]]; then
                ...
                echo ""
                local model_choice
                model_choice=$(prompt_choice "Select Gemini model" \
                ...
                success "Provider: Gemini, Model: $LLM_MODEL"
            fi
            ;;
        4|*)
